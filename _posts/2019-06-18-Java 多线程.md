...
### 多线程的内存模型
Java内存模型用来屏蔽各种硬件和操作系统的内存差异，达到跨平台的内存访问效果，JLS（Java language specification，java语言规范）定义了一个统一的内存管理模型JMM（Java Memory Model）。
...
多线程的内存模型，主内存（main memory），工作内存（working memory），在处理数据时，线程会把值从主内存load到工作内存，完成操作后再save回去。
